...
public class BieuDien_Form extends javax.swing.JFrame {
    ...
    ArrayList<String>[][] arrKetHop;      
    ArrayList<String>[][] arrCky;
    TreeMap<Integer, ArrayList<String>> grammar;
    ...
    private void initComponents() {
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(Jpnl, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        new CKY_Form(this.cau,this.dsCuPhap).setVisible(true);
        this.dispose();
...
//        });
    ...
    public void Init(String cau, String dsCuPhap){
        dsTu = arr_Tu(cau);
        soLuongTu = dsTu.length;
        arrKetHop=arr_Cky(soLuongTu);
        arrCky = arr_Cky(soLuongTu);
        grammar = TachCuPhap(dsCuPhap);
    }
    private void Load() {
    ...
    public void DrawCKY(){
        ...
        for (int i = 0; i < soLuongTu; i++) {
            for (int j = 0; j < soLuongTu; j++) {
                String text="";
                for(int k=0;k<arrCky[i][j].size();k++){
                    if(!text.equals("")){
                        text=text+","+arrCky[i][j].get(k)+arrKetHop[i][j].get(k);
                    }else{
                        text=text+arrCky[i][j].get(k)+arrKetHop[i][j].get(k);
                    }
    ...
    public void CKY() {
        for (int i = 0; i < soLuongTu; i++) {
            ...
            for(int lt=0;lt<left.size();lt++){
                arrCky[i][i].add(left.get(lt));
                arrKetHop[i][i].add("");
            }
            for (int j = i - 1; j >= 0; j--) {
                for (int k = j; k < i; k++) {
                    for(int s1=0;s1<arrCky[k][j].size();s1++){
                       for(int s2=0;s2<arrCky[i][k+1].size();s2++ ){
                           if(!arrCky[k][j].get(s1).equals("")&& !arrCky[i][k+1].get(s2).equals("")){
                               ArrayList<String> ntl=getNonTerLeft(arrCky[k][j].get(s1),arrCky[i][k + 1].get(s2),grammar);
                               for(int ntlItem =0;ntlItem<ntl.size();ntlItem++){
                                   arrCky[i][j].add(ntl.get(ntlItem));
                                   int x1=j;
                                   int y1=k+1;
                                   int x2=k+1;
                                   int y2=i+1;
                                   arrKetHop[i][j].add("[("+x1+","+y1+")+("+x2+","+y2+")]");
                               }
                           }
                       }
                    }
